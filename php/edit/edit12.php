...
include("db2.php");
	...
	if(empty($sim_card_number) || empty($MB_max)) {	
		...
		//updating the table
		$result = mysqli_query($mysqli, "UPDATE `ProCat_ID` SET `sim_card_number`='$sim_card_number',`MB_max`='$MB_max' WHERE `Serial_Number` LIKE '018E22DC44F1000003'");
...
//selecionar pelo ID
$result = mysqli_query($mysqli, "SELECT * FROM `ProCat_ID` WHERE `Serial_Number` LIKE '018E22DC44F1000003'");
...
<body>
  ...
  <h3>UPC Aveiro</h3></br>
  <form action="edit12.php" method="post" name="form1">
