...
include("db2.php");
	...
	if(empty($sim_card_number) || empty($MB_max)) {	
		...
		//updating the table
		$result = mysqli_query($mysqli, "UPDATE `ProCat_ID` SET `sim_card_number`='$sim_card_number',`MB_max`='$MB_max' WHERE `Serial_Number` LIKE '0411180A3E26000006'");
...
//selecionar pelo ID
$result = mysqli_query($mysqli, "SELECT * FROM `ProCat_ID` WHERE `Serial_Number` LIKE '0411180A3E26000006'");
...
<body>
  ...
  <h3>UAR-2</h3></br>
  <form action="edit22.php" method="post" name="form1">
