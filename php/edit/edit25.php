...
include("db2.php");
	...
	if(empty($sim_card_number) || empty($MB_max)) {	
		...
		//updating the table
		$result = mysqli_query($mysqli, "UPDATE `ProCat_ID` SET `sim_card_number`='$sim_card_number',`MB_max`='$MB_max' WHERE `Serial_Number` LIKE '0411180A3E26000009'");
...
//selecionar pelo ID
$result = mysqli_query($mysqli, "SELECT * FROM `ProCat_ID` WHERE `Serial_Number` LIKE '0411180A3E26000009'");
...
<body>
  ...
  <h3>UAR-7</h3></br>
  <form action="edit25.php" method="post" name="form1">
