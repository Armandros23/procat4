...
include("db2.php");
	...
	if(empty($sim_card_number) || empty($MB_max)) {	
		...
		//updating the table
		$result = mysqli_query($mysqli, "UPDATE `ProCat_ID` SET `sim_card_number`='$sim_card_number',`MB_max`='$MB_max' WHERE `Serial_Number` LIKE '032E280C4321000002'");
...
//selecionar pelo ID
$result = mysqli_query($mysqli, "SELECT * FROM `ProCat_ID` WHERE `Serial_Number` LIKE '032E280C4321000002'");
...
<body>
  ...
  <h3>UPC Póvoa</h3></br>
  <form action="edit11.php" method="post" name="form1">
